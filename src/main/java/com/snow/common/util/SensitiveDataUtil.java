...
public class SensitiveDataUtil {
	...
	private static final Pattern ID_CARD_PATTERN = Pattern
			.compile(ID_CARD_REGEXP);
	/** 银行卡号匹配模式 */
	private static final Pattern BANK_CARD_PATTERN = Pattern
			.compile(BANK_CARD_REGEXP);
	/** 手机或者固定电话匹配模式 */
	private static final Pattern PHONE_TEL_PATTERN = Pattern
			.compile(PHONE_TEL_REGEXP);
	...
	/**
	 * 对登陆帐号进行部分隐藏处理：如果是手机号，显示前3和后4位；如果是email，只显示用户名的前3位+*+@域名；如果都不是，显示前3和后4位
	 ...
	 */
	public static String logonIdHide(final String logonId) {
		if (!needHide())
			return logonId;
	...
	/**
	 * 对登陆帐号进行部分隐藏处理，适用于短信。<br/>
	 * 
	 * @param logonId
	 *            待部分隐藏处理的,可能是手机号或email。
	 * @return 如果hideFlag为true，返回按规范进行部分隐藏后的登录号。
	 */
	public static String logonIdHideSMS(final String logonId) {
		if (!needHide())
			return logonId;
	 ...
	 * @return 如果hideFlag为true，中推荐的电话号码部分展示字符串；否则返回原数据。
	 */
	public static String phoneOrTelNoHide(final String phoneOrTelNo,
		...
		int frontCharNum = (tmp.charAt(0) == '1') ? 3 : 4; // phoneOrTelNo以1开头，认为是手机号，前3，否则前4
		if (tmp.indexOf("-") > 0) { // 是固定电话号码
			frontCharNum = tmp.indexOf("-") + 1;
		}
		return customizeHide(tmp, frontCharNum, 4, tmp.length() - 4
				- frontCharNum);
	}
	...
	/**
	 * 手机号码通用隐藏规则（包括港澳台地区），隐藏中间四位 适用于网站以及客户端 <li>
	 * SensitiveDataUtil.cellphoneHide("13071835358") = 130****5358 <li>
	 * SensitiveDataUtil.cellphoneHide("3071835358") = 30****358 <li>
	 * SensitiveDataUtil.cellphoneHide("071835358") = 07****58 <li>
	...
	public static String cellphoneHide(final String cellphone) {
		...
		return customizeHide(tmp, notHideNum / 2, notHideNum - notHideNum / 2,
				4);
	}
	 ...
	 *            敏感数据类型，值可为：
	 *            <code>SensitiveDataUtil.BANKCARDNO_DATA<code>、<code>SensitiveDataUtil.IDCARDNO_DATA<code>、<code>SensitiveDataUtil.PHONENO_DATA<code>、<code>SensitiveDataUtil.EMAIL_DATA<code>或<code>SensitiveDataUtil.UNKNOWN_DATA<code>。
	 * @return 如果hideFlag为true，返回过滤掉敏感数据后的字符串；否则返回原数据。
	 */
	public static String filterHide(final String sourceStr,
			final String tagBegin, final String tagEnd,
			final int sensitiveDataType) {
		if (!needHide())
			return sourceStr;
		...
		while (begin != -1 && end != -1) {
			// 说明存在要求的节点
			target = target.append(tmp.toString().toCharArray(), 0, begin
					+ tagBegin.length());
			// 加入过滤字符
			String coverReplace = "";
			switch (sensitiveDataType) {
			case BANKCARDNO_DATA:
				coverReplace = bankCardNoHide(tmp.substring(
						begin + tagBegin.length(), end));
				break;
			case IDCARDNO_DATA:
				coverReplace = idCardNoHide(tmp.substring(
						begin + tagBegin.length(), end));
				break;
			case PHONENO_DATA:
				coverReplace = phoneOrTelNoHide(tmp.substring(
						begin + tagBegin.length(), end));
				break;
			case EMAIL_DATA:
				coverReplace = emailHide(tmp.substring(
						begin + tagBegin.length(), end));
				break;
			default:
				coverReplace = defaultHide(tmp.substring(
						begin + tagBegin.length(), end));
				break;
			}
	 ...
	 * 显示首/尾各1位，中间加**
	 * 
	 * <li>SensitiveDataUtil.taobaoNickHide("中华") = 中**华 <li>
	 * SensitiveDataUtil.taobaoNickHide("中华人") = 中**人 <li>
	 * SensitiveDataUtil.taobaoNickHide("中华人民") = 中**民 <li>
	 ...
	 * @return 部分隐藏后的昵称
	 */
	public static String nickNameHide(final String sensitiveData) {
		if (!needHide())
	 ...
	 * @return 部分隐藏的敏感数据字符串
	 */
	public static String customizeHide(final String sensitiveData,
			final int frontCharNum, final int tailCharNum,
